/*
    $Id: I2L_menus.j 474 2011-01-28 13:54:24Z br $

    Copyright (C) 2000-8 Richard Bornat
     
        richard@bornat.me.uk
		
	Modified for Logic Class by H.Peter Gumm (gumm@uni-marburg.de)
...
MENU Backward IS
    ...
    ENTRY   "→ intro (Unterbeweis)"    IS BackwardOnlyA (QUOTE (_A→_B)) 
                                                    (Noarg "→ intro" "→ intro") "→ intro" "A→B"
    ENTRY   "∨ intro-Links"     IS BackwardOnlyC (QUOTE (_A∨_B)) 
                                                    (Noarg (LAYOUT "∨ intro" (0) "∨ intro(L)" fstep) "∨ intro") "∨ intro" "A∨B"
    ENTRY   "∨ intro-Rechts"    IS BackwardOnlyC (QUOTE (_A∨_B)) 
                                                    (Noarg (LAYOUT "∨ intro" (0) "∨ intro(R)" fstep) "∨ intro") "∨ intro" "A∨B"
    ENTRY   "¬ intro "  IS BackwardOnlyA (QUOTE (¬_A)) 
                                                    (WITHARGSEL "¬ intro") "¬ intro" "¬A" 
    ENTRY   "∀ intro ( 'fest aber.beliebig' )" IS BackwardOnlyA (QUOTE (∀_x._A)) 
                                                    (Noarg "∀ intro" "∀ intro") "∀ intro" "∀x.A" 
    ENTRY   "∃ intro (Term einsetzen)"      "∃ intro backward"
    
    ENTRY "truth" IS BackwardOnlyA ⊤(Noarg "truth" "truth") "truth" ⊤
    
    SEPARATOR
    
    ENTRY   "indirekter Beweis"   
                                             IS BackwardOnlyA (QUOTE _A) 
                                                    (Noarg "contra (classical)" "contra (classical)") "contra (classical)" "A"

/*    SEPARATOR

    ENTRY   "⊥ elim (ExFalso)"                   IS BackwardOnlyA (QUOTE _A) 
...
MENU Forward IS
    ENTRY   "∧ elim-Links"      IS Forward (QUOTE (_A∧_B)) (ForwardCut 0 (Noarg (LAYOUT "∧ elim" (0) "∧ elim(L)") "∧ elim"))  "∧ elim" "∧ intro" "A∧B"
    ENTRY   "∧ elim-Rechts"     IS Forward (QUOTE (_A∧_B)) (ForwardCut 0 (Noarg (LAYOUT "∧ elim" (0) "∧ elim(R)") "∧ elim")) "∧ elim" "∧ intro" "A∧B"
    ENTRY   "→ elim (ModusPonens)"                   IS "→ elim forward"
    ENTRY   "∨ elim (Fallunterscheidung)"    IS Forward (QUOTE (_A∨_B)) 
                                                        (Noarg ("targeted forward" (ForwardUncut 0 "∨ elim") "∨ elim") "∨ elim") 
                                                        "∨ elim" "∨ intro" "A∨B"
    ENTRY   "¬ elim"                        IS "¬ elim forward"
    ENTRY   "∀ elim (Term einsetzen)"       IS "∀ elim forward"
    ENTRY   "∃ elim (liefert Zeugen)"    IS Forward (QUOTE (∃_x._A)) 
                                                            (Noarg ("targeted forward" (ForwardUncut 0 "∃ elim") "∃ elim") "∃ elim") 
                                                            "∃ elim" "∃ intro" "∃x.A"
 
    ENTRY   "⊥ elim (ExFalso)"                  IS Forward ⊥  (Noarg ("targeted forward" "contra (constructive) forward" "contra (constructive)") 
                                                                  "contra (constructive)")
                                                           "contra (constructive)" "¬ elim" ⊥
/*    SEPARATOR
    ENTRY   "∧ intro (H1 ∧ H2)"                      IS "∧ intro forward"

    ENTRY   "∨ intro (H ∨ ?)"      IS "∨ introforward" "∨ intro(L)"
    ENTRY   "∨ intro (? ∨ H)"       IS "∨ introforward" "∨ intro(R)"
    
*/

    /* removed -- see comment on the tactic below
    ENTRY   "∃ intro (braucht Zeugen)"      IS "∃ intro forward"
